    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/75 backdrop-blur-sm p-3 sm:p-4 animate-fade-in font-mono">
      <div className="w-full max-w-md p-4 sm:p-6 rounded-2xl bg-[#18181b] border border-neutral-800 shadow-2xl flex flex-col items-center select-none">
        ...
        <div className="text-center mb-5">
          ...
          <h2 className="text-xl font-bold text-neutral-100 mt-0.5">결과 요약</h2>
          {isRecordMode && (
            <div className="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-medium">
        ...
        {/* 세부 통계 */}
        <div className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-neutral-900/40 border border-neutral-800/60 text-xs text-neutral-400 mb-5">
          <span>총 입력: <b className="text-neutral-200">{result.totalKeys}</b></span>
          <span>정타: <b className="text-emerald-400">{result.correctKeys}</b></span>
          <span>오타: <b className="text-rose-400">{result.wrongKeys}</b></span>
        </div>
          ...
          <div className="w-full mb-5">
            ...
            <div className="flex flex-wrap gap-2">
              {result.mistakes.slice(0, 3).map((m, idx) => (
                ...
                >
                  <span className="text-neutral-300 font-bold">{m.targetKey}</span>
                  <span className="text-neutral-500">→</span>
                  <span className="text-rose-400 font-bold">{m.pressedKey}</span>
