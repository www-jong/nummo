...
export const NicknameModal: React.FC<NicknameModalProps> = ({ isOpen, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/80 backdrop-blur-sm p-4 font-mono animate-fade-in">
      <div className="w-full max-w-md p-6 rounded-2xl bg-white dark:bg-[#18181b] border border-neutral-200 dark:border-neutral-700 shadow-2xl flex flex-col gap-5">
        {/* 헤더 */}
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-500 dark:bg-amber-400 animate-pulse" />
            <h2 className="text-lg font-bold text-neutral-900 dark:text-neutral-100">NUMMO 닉네임 설정</h2>
          </div>
          <p className="text-xs text-neutral-600 dark:text-neutral-400 leading-relaxed">
            NUMMO는 이메일이나 실명을 일절 저장하지 않습니다.<br />
            기록과 랭킹에 표시될 <strong className="text-neutral-800 dark:text-neutral-200">고유 닉네임</strong>을 설정해주세요.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <label htmlFor="nickname-input" className="text-xs text-neutral-600 dark:text-neutral-400 font-medium">
              닉네임 (2~12자, 한글/영문/숫자)
            </label>
            <div className="flex items-center gap-2">
              <input
          ...
          <button
            ...
            className="w-full py-3 rounded-xl font-bold text-sm transition-all shadow-lg mt-2 bg-amber-500 hover:bg-amber-400 text-neutral-950 disabled:bg-neutral-200 dark:disabled:bg-neutral-800 disabled:text-neutral-400 dark:disabled:text-neutral-500 disabled:cursor-not-allowed"
          >
