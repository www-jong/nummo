  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {showFingerGuide && currentFinger && (
        <div className="mb-2 sm:mb-4 flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-neutral-200 dark:border-neutral-800 bg-white/90 dark:bg-neutral-900/90 text-xs tracking-wider transition-all duration-150 shadow-md">
      ...
      <div
        className="nummo-numpad grid grid-cols-4 grid-rows-5 gap-1.5 sm:gap-2 p-3 sm:p-4 rounded-2xl bg-white/95 dark:bg-neutral-900/80 border border-neutral-200 dark:border-neutral-800/80 shadow-xl dark:shadow-2xl backdrop-blur-sm select-none transition-colors"
      >
        {NUMPAD_KEYS.map((k) => {
          ...
          let bgClass = 'bg-neutral-100/80 hover:bg-neutral-200/90 dark:bg-neutral-800/60 dark:hover:bg-neutral-800 text-neutral-800 dark:text-neutral-300';
          let shadowClass = '';

          if (isPressed) {
            bgClass = 'bg-neutral-300 dark:bg-neutral-700 text-neutral-950 dark:text-white scale-[0.96]';
            borderClass = 'border-neutral-400 dark:border-neutral-500';
          } else if (isTarget) {
            bgClass = 'bg-amber-100/90 dark:bg-neutral-800 text-amber-950 dark:text-white animate-pulse';
            borderClass = 'border-amber-500 dark:border-amber-400/90';
            shadowClass = 'shadow-[0_0_14px_rgba(245,158,11,0.35)]';
          }
            ...
            >
              ...
              <span
                className={`transition-colors ${
                  isLongLabel
                    ? 'text-[11px] font-semibold tracking-tight text-neutral-500 dark:text-neutral-400'
                    : 'text-base font-medium'
                } ${isTarget ? 'font-bold text-amber-600 dark:text-amber-300' : ''}`}
              >
                {k.label}
              </span>
              ...
              {k.code === 'Numpad5' && (
                <div className="absolute bottom-2.5 w-2 h-0.5 rounded-full bg-neutral-400 dark:bg-neutral-500" />
              )}
